...
- `sort(first,last,comparator)` sorts elements present in range `[first,last)`. Default order is ascending; we can use the `greater<Type>()` comparator for descending
- `binary_search(first,last,val)` returns `true` if any element in the range `[first,last)` is equivalent to `val`
